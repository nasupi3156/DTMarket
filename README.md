...
今回はデフォルトで書いてありますが、環境に合わせて自由に設定してください。
```php
//（Docker/ローカル共通設定）
...
.envファイルは、デフォルトで作成してありますが`database_name`と`username`と`password`など環境に合わせて自由に設定してください。
...
```diff
ローカル環境でデータベースを利用するには、MAMP（Mac）やXAMPP（Windows）などの開発環境をインストールしてください。詳細なインストール手順については、各公式サイトを参考にしてください。
```
...
mysqlフォルダー内の`DTshopping.sql`にあるテーブル情報を順番にインポートしてください。

**注意：**  一度に沢山をインポートするとエラーが起きる可能性が高いので、一つずつテーブルをインポートするのがオススメです。
